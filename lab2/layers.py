...
class FC(Layer):
    ...
    def forward(self, inputs):
        ...
        self.input_shape = inputs.shape
        self.inputs = inputs
        return inputs @ self.weights.T + self.bias.reshape(1, -1)
...
class ReLU(Layer):
    ...
    def backward_inputs(self, grads):
        ...
        Returns:
          ndarray of shape (N, C, H, W).
        """
        d_relu = np.maximum(0, self.inputs)
        d_relu[d_relu != 0] = 1
        return grads * d_relu
...
class SoftmaxCrossEntropyWithLogits():
    ...
    def backward_inputs(self, x, y):
        ...
        N = len(x)
        expo = np.exp(x)
        softmax = expo / np.sum(expo, axis=1).reshape(-1, 1)
        return 1/N * (softmax - y)
...
class L2Regularizer():
    ...
    def forward(self):
        ...
        """
        return self.weight_decay/(2*self.weights.shape[0]) * np.linalg.norm(self.weights.reshape(-1))
    ...
    def backward_params(self):
        ...
        """
        grad_weights = self.weight_decay/self.weights.shape[0] * self.weights
